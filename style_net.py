...
class StyleNet():
    ...
    def apply_style(self, content_image, style_image, display=False, iterations=1000, **kwargs):
        model = load_vgg16((None, None, 3), include_top=False)

        content_outputs = [x.output for x in model.layers if x.name == self.content_layer]
        content_func = K.function(model.inputs, content_outputs)
        content_features = content_func([np.expand_dims(content_image, axis=0)])

        style_outputs = [x.output for x in model.layers if x.name in self.style_layers]
        style_func = K.function(model.inputs, style_outputs)
        ...
        c_loss, t_loss, s_loss = self._neural_style_loss(model, content_features, style_features, generated_image, **kwargs)
        loss = c_loss # c_loss + t_loss + s_loss

        optimizer = keras.optimizers.adam(lr=1.)
        updates = optimizer.get_updates(loss=loss, params=model.inputs)
        f = K.function(model.inputs, [c_loss, t_loss, s_loss], updates=updates)
    ...
    def _neural_style_loss(self, model, content_features, style_features, generated_image, content_weight=5., tv_weight=1e2, style_weight=5e2):
        generated_features = model.get_layer(self.content_layer).output
        content_loss = K.mean(K.square(content_features[0] - generated_features[0]))

        tv_loss = self._total_variation_loss(generated_image)

        style_loss = 0.
        generated_features = [x.output for x in model.layers if x.name in self.style_layers]
